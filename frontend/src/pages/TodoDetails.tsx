    ...
    <Container>
      ...
      <Card title={todo.title}> 
        <TodoItemContainer>
          ...
          <div>Video Link: {
            todo.video_name && (
              <a href={`/${todo.id}/video/${todo.video_name}`}>{`${window.location.origin}/${todo.id}/video/${todo.video_name}`}</a>
            )}
          </div>
          ...
          {todo.status === TodoStatus.completed && (<Button label="Play Video" onClick={openPoupVideo(todo.title, todo.video_name)}/>)}
        </TodoItemContainer>
      </Card>
    </Container>
  );
