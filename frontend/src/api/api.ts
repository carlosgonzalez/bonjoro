...
import { TodoListResponse, AddTodoRequest, DeleteTodoResponse, SingleTodoResponse, UpdateTodoRequest } from '../types/Todo';
...
export async function getTodo(id: string|null) {
  if(!id) return;
  try {
    let res = await axios.get<SingleTodoResponse>(`${config.api}api/todo/${id}`);
    let data = res.data;
    return data;
  } catch (error) {
